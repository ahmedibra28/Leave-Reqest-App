...
function EmployeeForm({handleChange, handleSubmit, values, errors, employees, leave_types}) {
    return (
      ...
      <hr className="mt-0"/>
            ...
            <div className="input-group">
              ...
              <select name="employee" onChange={handleChange} value={values.employee} className="form-control py-2">
                <option value="" disabled>Emoloyee ID...</option>
                {
                  employees && employees.map(employee => {
                  return <option key={employee.id} value={employee.id}>{employee.emp_id}</option>
                  })
                }
              </select>
          ...
          <div className="input-group">
              ...
              <select name="leave" onChange={handleChange} value={values.leave} className="form-control py-2">
                <option value="" disabled>Leave Type...</option>
                {
                  leave_types && leave_types.map(leave_type => {
                  return <option key={leave_type.id} value={leave_type.id}>{leave_type.name}</option>
                  })
                }
              </select>
            </div>
            ...
            <div className="input-group">
              <span className="input-group-text" id="basic-addon1">
              <svg viewBox="0 0 20 20" fill="currentColor" className="calendar w-6 h-6"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" /></svg>
              </span>
              <input
                name="start_date"
...
<div className="input-group">
              <span className="input-group-text" id="basic-addon1">
              <svg viewBox="0 0 20 20" fill="currentColor" className="dots-circle-horizontal w-6 h-6"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd" /></svg>
              </span>
              <textarea
              rows="5"
              cols="30"
