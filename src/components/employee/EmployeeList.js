...
const EmployeeList = ({handleUpdate, employees, deleteEmployee, departmentIdToName}) => {
                ...
                <tbody>
                    {
                        employees && employees.map((employee) => {
                            return (
                                <tr key={employee.id} id={employee.id % 2 === 0 ? "orange" : "green"}>
                                    <td>{employee.emp_id}</td>
                                    <td>{employee.name}</td>
                                    <td>{employee.mobile}</td>
                                    <td>{departmentIdToName(employee.department)}</td>
                                    <td> 
                            <button onClick={() => handleUpdate(employee)} className="btn btn-outline-info btn-sm">
